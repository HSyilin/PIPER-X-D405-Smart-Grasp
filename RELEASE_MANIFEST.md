...
- 测试盒感知配置启用固定尺寸模式：跳过视觉 `SIZE_MISMATCH`，对下游固定发布
  实测 `76.5 x 106.5 x 30 mm`，同时保留深度定位、TF、位姿稳定和全部执行门。
- 根据现场连续检测数据，为每个 track 增加尺寸中值、尺寸离群剔除和尺寸极差
  门控；测试盒使用 15 帧窗口。精确图像时间 TF 等待参数化，VMware 测试配置
...
- 新增 `test_param_tuner.py` 和尺寸稳定性覆盖，当前直接 pytest 回归结果为
  `16 passed`；
